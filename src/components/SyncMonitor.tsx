...
export function SyncMonitor() {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (s.status === "completed") {
          ...
          setTimeout(() => { setExpanded(false); setLogs([]); lastCountRef.current = 0; }, 5000);
  ...
  const pct = total > 0 ? Math.round((processed / total) * 100) : 0;
  const elapsed = lastStatus?.started_at 
    ? Math.round((Date.now() - new Date(lastStatus.started_at).getTime()) / 1000 / 60)
    : 0;
    ...
    <>
        ...
        >
          <div className="flex items-center gap-2 w-full">
            ...
            <span className="flex-1 text-left font-bold" style={{ fontFamily: "var(--font-heading)" }}>
              {isRunning ? `Sincronizando ${active.dateStr}` : lastStatus?.status === "failed" ? "Falhou" : "Concluído"}
            </span>
      ...
      {expanded && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center pointer-events-none"
          ...
          <div className="pointer-events-auto w-full sm:max-w-2xl sm:m-4 mx-2 mb-20 sm:mb-4 shadow-2xl"
